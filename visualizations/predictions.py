...
def load_hcc_summary():
    return pd.read_parquet(DATA / "hcc_summary.parquet")


@st.cache_data
def load_ml_dataset():
    return pd.read_parquet(DATA / "hcc_ml_dataset.parquet")


@st.cache_data
def load_patient_conditions(person_id):
    df = pd.read_parquet(DATA / "patient_conditions.parquet")
    return (
        df[df["person_id"] == person_id]
        .sort_values("recorded_date", ascending=False)
        .head(15)
        [["normalized_code", "normalized_description", "condition_type", "recorded_date"]]
    )


@st.cache_data
def load_patient_hcc_suspects(person_id):
    df = pd.read_parquet(DATA / "hcc_list.parquet")
    return (
        df[df["person_id"] == person_id]
        .sort_values("suspect_date", ascending=False)
        .head(10)
        [["hcc_code", "hcc_description", "reason", "contributing_factor", "suspect_date"]]
    )
...
with tab1:
    ...
    if df_hcc.empty:
        ...
        age_range = st.slider("Filter by age range", min_value=age_min, max_value=age_max,
                               value=(age_min, age_max), key="t1_age")

        df_view = df_hcc[df_hcc["patient_age"].between(age_range[0], age_range[1])].copy()
        ...
        with col_a:
            ...
            df_age = (
                ...
                .mean().round(1).rename(columns={"suspecting_gaps": "avg_gaps"})
            )
            st.altair_chart(
        ...
        with col_b:
            ...
            band_colors = {
                "Low (0–4)": "#2ca02c", "Medium (5–14)": "#f0c05a",
                "High (15–29)": "#ff7f0e", "Very high (30+)": "#d62728",
            }
            st.altair_chart(
                alt.Chart(df_band).mark_bar().encode(
                    x=alt.X("risk_band:N", title="Risk band", sort=band_order),
                    y=alt.Y("patients:Q", title="Number of patients"),
                    color=alt.Color(
                        "risk_band:N",
                        scale=alt.Scale(domain=list(band_colors.keys()), range=list(band_colors.values())),
                        legend=None,
                    ),
                    column=alt.Column("patient_sex:N", title="Sex", header=alt.Header(labelAngle=0)),
...
with tab2:
    ...
    if result is None:
        ...
        with col_a:
            ...
            st.altair_chart(
                alt.Chart(imps).mark_bar().encode(
                    x=alt.X("importance:Q", title="Importance",
                             scale=alt.Scale(domain=[0, imps["importance"].max() * 1.1])),
        ...
        with col_b:
            ...
            cm_text = heatmap.mark_text(fontSize=14).encode(
                text="label:N", color=alt.value("white"),
            )
    ...
    if df_hcc_all.empty:
        ...
        if result is not None:
            ...
            if not patient_row.empty:
                prob = result["model"].predict_proba(patient_row[FEATURE_NAMES].fillna(0).values)[0][1]
                st.markdown("---")
...
""")
        ...
        tier_filter = st.multiselect("Filter by risk tier", ["High", "Medium", "Low"],
                                      default=["High", "Medium"], key="rec_tier")
